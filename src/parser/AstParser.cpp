...
bool AstParser::stmtList(int parent, bool newElement) {
  ...
  int safe = this->index;
  return (this->stmt(par) && this->stmtList(par, false)) ||
         (this->index = safe, this->stmt(par)) || (this->index = safe, true);
}

bool AstParser::isMultToken(int parent) {
  int safe = this->index;
  return (this->index = safe,
          this->checkToken(Token_OperatorMultiply, parent)) ||
...
bool AstParser::primary(int parent, bool newElement) {
  ...
  int safe = this->index;
  return (this->index = safe, this->checkToken(Token_Identifier, par)) ||
         (this->index = safe, this->checkToken(Token_Int, par)) ||
         (this->index = safe, this->checkToken(Token_Double, par)) ||
         (this->index = safe, this->checkToken(Token_BraceLeft, par) &&
                                  this->expr(par) &&
                                  this->checkToken(Token_BraceRight, par)) ||
         (this->index = safe,
          this->checkToken(Token_OperatorPlus, par) && this->primary(par)) ||
         (this->index = safe, this->checkToken(Token_OperatorMinus, par) &&
                                  this->primary(par, false)) ||
         (this->index = safe, this->checkToken(Token_OperatorNot, par) &&
                                  this->primary(par, false));
}
...
bool AstParser::mulExpr(int parent, bool newElement) {
  ...
  int safe = this->index;
  return (this->primary(par) && this->isMultToken(par) &&
          this->mulExpr(par, false)) ||
         (this->index = safe, this->primary(par));
}
...
bool AstParser::addExpr(int parent, bool newElement) {
  ...
  int safe = this->index;
  return (this->mulExpr(par) && this->isAddToken(par) &&
          this->addExpr(par, false)) ||
         (this->index = safe, this->mulExpr(par));
}
...
bool AstParser::isEqualToken(int parent) {
  ...
  return (this->index = safe, this->checkToken(Token_OperatorEq, parent)) ||
         ...
         (this->index = safe,
          this->checkToken(Token_OperatorSmaller, parent)) ||
         (this->index = safe,
          this->checkToken(Token_OperatorSmallerEq, parent)) ||
         (this->index = safe,
          this->checkToken(Token_OperatorGreater, parent)) ||
         (this->index = safe,
          this->checkToken(Token_OperatorGreaterEq, parent));
}

bool AstParser::eqExpr(int parent, bool newElement) {
  int par = this->addNode(AstEqExpr, parent, newElement);
  int safe = this->index;
  return (this->addExpr(par) && this->isEqualToken(par) &&
          this->eqExpr(par, false)) ||
         (this->index = safe, this->addExpr(par));
}

bool AstParser::andExpr(int parent, bool newElement) {
  int par = this->addNode(AstAndExpr, parent, newElement);
  int safe = this->index;
  return (this->eqExpr(par) && this->checkToken(Token_OperatorLogicAnd, par) &&
          this->andExpr(par, false)) ||
         (this->index = safe, this->eqExpr(par));
}

bool AstParser::expr(int parent, bool newElement) {
  int par = this->addNode(AstExpr, parent, newElement);
  int safe = this->index;
  return (this->andExpr(par) && this->checkToken(Token_OperatorLogicOr, par) &&
          this->expr(par, false)) ||
         (this->index = safe, this->andExpr(par));
}

bool AstParser::parExpr(int parent, bool newElement) {
  int par = this->addNode(AstParExpr, parent, newElement);
  return this->checkToken(Token_BraceLeft, par) && this->expr(par) &&
         this->checkToken(Token_BraceRight, par);
}
...
bool AstParser::stmtWhile(int parent, bool newElement) {
  ...
  return (this->index = safe, this->checkToken(Token_KeywordWhile, par) &&
                                  this->parExpr(par) && this->stmt(par));
}
...
bool AstParser::stmtIf(int parent, bool newElement) {
  ...
  return (this->index = safe, this->checkToken(Token_KeywordIf, par) &&
                                  this->parExpr(par) && this->stmt(par) &&
                                  this->checkToken(Token_KeywordElse, par) &&
                                  this->stmt(par)) ||
         (this->index = safe, this->checkToken(Token_KeywordIf, par) &&
                                  this->parExpr(par) && this->stmt(par));
}
...
bool AstParser::stmtDeclaration(int parent, bool newElement) {
  ...
  return (this->index = safe, this->checkToken(Token_KeywordVar, par) &&
                                  this->checkToken(Token_Identifier, par) &&
                                  this->checkToken(Token_OperatorAssign, par) &&
                                  this->expr(par) &&
                                  this->checkToken(Token_Semilicon, par)) ||
         (this->index = safe, this->checkToken(Token_KeywordVar, par) &&
                                  this->checkToken(Token_Identifier, par) &&
                                  this->checkToken(Token_Semilicon, par));
}
...
bool AstParser::stmt(int parent, bool newElement) {
  ...
  return (this->index = safe, this->checkToken(Token_Semilicon, par)) ||
         (this->index = safe, this->checkToken(Token_SwiftLeft, par) &&
                                  this->stmtList(par) &&
                                  this->checkToken(Token_SwiftRight, par)) ||
         (this->index = safe, this->stmtAssigment(par)) ||
         (this->index = safe, this->stmtWhile(par)) ||
         (this->index = safe, this->stmtIf(par)) ||
         (this->index = safe, this->stmtDeclaration(par));
}
...
bool AstParser::idList(int parent, bool newElement) {
  ...
  return this->isNextToken(Token_BraceRight) ||
         ...
         ((this->index = safe, this->checkToken(Token_Identifier, par) &&
                                   this->idList(par, false))) ||
         ((this->index = safe, this->checkToken(Token_Comma, par) &&
                                   this->checkToken(Token_Identifier, par) &&
                                   this->idList(par, false)));
}
...
bool AstParser::func(int parent, bool newElement) {
  ...
  return this->checkToken(Token_KeywordFunc, par) &&
         this->checkToken(Token_Identifier, par) &&
         this->checkToken(Token_BraceLeft, par) && this->idList(par) &&
         this->checkToken(Token_BraceRight, par) && this->stmt(par);
}
...
bool AstParser::root(int parent, bool newElement) {
  ...
  return (this->checkToken(Token_Eof, par) ||
          (this->index = safe, this->func(par) && this->root(par, false)));
}
...
AstElementTree *AstParser::getRootNode() { return this->node[1].el; }
