...
bool Tokenizer::isOperator(char c) { return inList<char>(oper, c); }

bool Tokenizer::isBrace(char c) { return inList<char>(brace, c); }

bool Tokenizer::isSpecialChar(char c) { return inList<char>(specialChars, c); }

bool Tokenizer::isEmptySpace(char c) { return inList<char>(emptySpace, c); }
...
Token *Tokenizer::createOperator(std::ifstream *pFile) {
  ...
  for (StringToOperator map : MappingOperators) {
    if (map.size == 1 && map.str[0] == c1) {
      opType = map.tok;
    } else if (map.size == 2 && map.str[0] == c1 && map.str[1] == c2) {
      opType = map.tok;
      pFile->get(c2);
      break;
    }
...
Token *Tokenizer::createBrace(std::ifstream *pFile) {
  ...
  switch (c) {
  case '(':
    return new Token(Token_BraceLeft);
  case ')':
    return new Token(Token_BraceRight);
  case '{':
    return new Token(Token_SwiftLeft);
  case '}':
    return new Token(Token_SwiftRight);
  default:
    break;
  }
  ASSERT(true, NULL);
...
Token *Tokenizer::parse(std::ifstream *pFile) {
  ...
  if (isdigit(c))
    ...
    return this->createSpecialChar(pFile);
